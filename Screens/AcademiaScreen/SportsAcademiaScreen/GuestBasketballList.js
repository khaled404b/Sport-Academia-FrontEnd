...
const GuestBasketballList = ({ navigation }) => {
  const [companies, setCompanies] = useState([]);
  const [searchTerm, setSearchTerm] = useState(''); // Add this state variable
  const fadeAnim = useState(new Animated.Value(0))[0];

  useEffect(() => {
    const fetchBasketballAcademies = async () => {
      try {
        const response = await fetch(`${API_URL}/getBasketballAcademies`);
        if (!response.ok) {
...
style={styles.backgroundImg}>
      ...
      <ScrollView contentContainerStyle={styles.scrollViewContent}>
        <Animated.View style={[styles.contentContainer, { opacity: fadeAnim }]}>
          <Text style={styles.title}>Basketball Academies</Text>
...
export default GuestBasketballList;
